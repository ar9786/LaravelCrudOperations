  ...
  <div class="thejumbotron pagebanner">
    ...
    <div class="page_name_wrap">
      <div class="page_name_data">
        <h3 class="text-uppercase">@lang('home.content31')</h3>
      </div>
    </div>
  </div>
  ...
  <div class="multiple_games pt-5 pb-5">
    <div class="container">
      <!--<div class="add_sport text-center mb-5">
        <button class="btn btn-primary pt-2 pb-2" id="addSport">Add camp and clinic</button>
      </div>-->
      <div id="related_search"></div>
      @if (@isset(Session::get('authData')['userId']))
      <div class="col_selector">
        <div class="sports_selected mb-5">
          <span>@lang('static.static23')</span>
        </div>
      </div>
      @endif
      <div class="row">
      @if (@isset(Session::get('authData')['userId']))
      @if(count($data['user_added_trmts']) > 0)
      @foreach ($data['user_added_trmts'] as $value)
        ...
        @endif
        @endif
        @if(count($data['related_added_trmts']) > 0)
        <div class="col_selector ml-3 mr-3 w-100">
          <div class="sports_selected mb-5">
            <span>@lang('static.static13')</span>
          </div>
        </div>
        
        @foreach ($data['related_added_trmts'] as $value)
        <div class="col-12 col-md-4">
          <div class="col_selector Tournament_images mb-5">
