...
$api = callAPI("POST","https://api.tap.company/v2/tokens",json_encode($data));

$get_token_id = json_decode($api);
echo '<pre>';
print_r($get_token_id);
echo $get_token_id->id; */
?>
<!DOCTYPE html>
<html>
<head>
...
<form id="form-container" method="post" action="/charge">
  ...
  <button id="tap-btn" onclick="hideBtn()">Submit</button>
</form>
</div>
<div id="loader" style='display:none;'>
<img src="/payment/loader.gif"/>
</div>
<script>
    function hideBtn()
    {
...
<script>
//var tap = Tapjsli('pk_test_WhawgZ7epdJyfAiqLktbK12o'); //testing
var tap = Tapjsli('pk_live_Os8Czw5vgK3oS1MkcLIZ2UHE'); //sandbox
//var tap = Tapjsli('pk_live_Os8Czw5vgK3oS1MkcLIZ2UHE'); //live
...
function sendTokenId(token){
	setTimeout(continueExecution(token), 5000)
	function continueExecution(token)
	{
	  	$token_id = token;
		$.ajax({
				url  : 'https://www.sportstrives.com/html/payment/send_token.php',
				type : 'post',
				data : {
					token_id  : $token_id,	
				},
